...
public class Board extends JPanel {
	...
	private void initBoard() {
		...
		setFocusable(true);
		setPreferredSize(new Dimension(Regular.WIDTH, Regular.HEIGHT));
		
		gameInit();
	}
	
	private void gameInit() {
		bricks = new Brick[Regular.NO_OF_BRICKS];
		...
		timer = new Timer(Regular.PERIOD, new GameCycle());
		timer.start();
	}
	...
	private void drawObjects(Graphics2D g2d) {
		...
		for(int i = 0; i < Regular.NO_OF_BRICKS; i++) {
	...
	private void gameFinished(Graphics2D g2d) {
		...
		g2d.setFont(font);
		g2d.drawString(message, (Regular.WIDTH - fontMetrics.stringWidth(message)) / 2, Regular.WIDTH / 2);
	}
	...
	private void checkCollision() {
		if(ball.getRect().getMaxY() > Regular.BOTTOM_EDGE) {
			stopGame();
		}
		
		for(int i = 0, j = 0; i < Regular.NO_OF_BRICKS; i++) {
			if(bricks[i].isDestroyed()) {
				j++;
			}
			
			if(j == Regular.NO_OF_BRICKS) {
				message = "Victory";
				stopGame();
			}
		}
